...
def prepare(
    ...
    force: bool = typer.Option(False, "--force", help="Re-download and overwrite the cache."),
):
...
def ingest(
    ...
    with_graph: bool = typer.Option(
        False, "--with-graph/--no-graph",
        help="Also extract the knowledge graph (needed for `all_three` graph facts). Slow.",
    ),
    backend: str | None = typer.Option(None, "--extractor", "-e", help="Graph extractor backend."),
):
    ...
    corpus = dataset.build_corpus(records)
    typer.echo(f"Ingesting {len(corpus)} paragraphs (graph={'on' if with_graph else 'off'})...")
...
def run(
    ...
    graph_ingested: bool = typer.Option(
        True, "--graph-ingested/--no-graph-ingested",
        help="Whether the graph was ingested (annotates the report only).",
    ),
):
    """Run the full comparison matrix and write JSON/CSV/markdown reports."""
    records = dataset.load_cached(n=n, seed=seed)
    ...
    typer.echo(f"Running evaluation over {len(records)} queries (seed={seed}, top_k={top_k})...")
    results = run_all(records, top_k=top_k, candidate_k=candidate_k, on_progress=_progress)
    payload = [r.to_dict() for r in results]

    meta = {"n": len(records), "seed": seed, "top_k": top_k, "graph_ingested": graph_ingested}
    paths = report.write_reports(payload, config.RESULTS_DIR, meta=meta)

    typer.echo("\n" + report.to_markdown(payload, meta))
    typer.echo(f"Reports written: {paths['json'].name}, {paths['csv'].name}, {paths['markdown'].name}")
...
def all(
    ...
    corpus = dataset.build_corpus(records)
    typer.echo(f"Prepared {len(records)} queries; ingesting {len(corpus)} paragraphs...")
    ...
    payload = [r.to_dict() for r in results]
    meta = {"n": len(records), "seed": seed, "top_k": top_k, "graph_ingested": with_graph}
    paths = report.write_reports(payload, config.RESULTS_DIR, meta=meta)
    typer.echo("\n" + report.to_markdown(payload, meta))
    typer.echo(f"Reports written to {config.RESULTS_DIR}")

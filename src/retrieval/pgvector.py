...
class VectorStore:
    def __init__(self, conn_str: str | None = None):
        self.conn_str = conn_str or settings.database_url
    ...
    def store_vector(self, chunks: Iterable[Mapping[str, Any]], conn_str: str | None = None) -> int:
        ...
        inserted = 0
        try:
            with psycopg.connect(conn_str or self.conn_str) as conn:
            ...
            LOGGER.exception("Failed inserting %s chunk(s) into hybrid_rag_chunks", len(rows))
...
class VectorRetriever:
    ...
    def retrieve_relevant_chunks(self, query_embedding, limit=3):
        LOGGER.info("Searching database for context...")
